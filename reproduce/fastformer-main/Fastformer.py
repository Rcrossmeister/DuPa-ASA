...
if __name__ == '__main__':
    ...
    with open('/home/hzj/NLP1/SentimentAnalysis/Data/ABSA_train.txt') as train_file:
        train_dataset = train_file.readlines()
        train_file.close()
    with open('/home/hzj/NLP1/SentimentAnalysis/Data/ABSA_test.txt') as test_file:
    ...
    label = []
    for row in dataset:
        text.append(jieba.lcut(row[:-3]))
    for row in dataset:
        if num_pattern.match(row[-2]):
    ...
    split_num = int(len(label)*0.8)
    train_index = index[:split_num]
    np.random.shuffle(train_index)
    test_index = index[split_num:]
