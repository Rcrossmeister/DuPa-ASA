...
bert_model = BertModel.from_pretrained("/home/sy/code/DUPA-ASA/bert-base-uncased").to(torch.device('cuda:{}'.format(int(local_rank)) if torch.cuda.is_available() else 'cpu'))
bert_tokenizer = BertTokenizer.from_pretrained("/home/sy/code/DUPA-ASA/bert-base-uncased")
extract_model = BertForQuestionAnswering.from_pretrained("/home/sy/code/DUPA-ASA/bert-base-uncased").to(torch.device('cuda:{}'.format(int(local_rank)) if torch.cuda.is_available() else 'cpu'))
...
class Config(object):
    ...
    def __init__(self, dataset, embedding):
        ...
        self.extract_model = BertForQuestionAnswering.from_pretrained("/home/sy/code/DUPA-ASA/bert-base-uncased").to(torch.device('cuda:{}'.format(int(local_rank)) if torch.cuda.is_available() else 'cpu'))
